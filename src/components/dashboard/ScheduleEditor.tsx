...
export function ScheduleEditor({ value, onChange }: Props) {
  ...
  const setType = (t: ScheduleType) => {
    ...
    if (t === "daily") onChange({ type: "daily" });
      ...
      onChange({ type: "weekdays", weekdays: value.weekdays ?? [1, 2, 3, 4, 5] });
      ...
      onChange({ type: "oneoff", date: value.date ?? new Date().toISOString().slice(0, 10) });
  };
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            onChange={(e) =>
              onChange({
                ...value,
                monthDay: e.target.value === "last" ? "last" : parseInt(e.target.value, 10),
              })
            }
