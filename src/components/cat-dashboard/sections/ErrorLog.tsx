...
export default function ErrorLog() {
  ...
  const openEdit = (e: ErrorEntry) => {
    ...
    setForm({
      ...
      section: e.section,
      topic: e.topic || "",
      mock_number: e.mock_number || null,
    });
    setShowForm(true);
  };
  ...
  const filtered = errors.filter((e) => {
    ...
    if (
      filters.search &&
      !`${e.question} ${e.topic} ${e.fix}`.toLowerCase().includes(filters.search.toLowerCase())
    )
      return false;
    return true;
  });
  ...
  const causeCounts = ["Concept Gap", "Silly Mistake", "Timing"].map((c) => ({
    cause: c,
    count: errors.filter((e) => e.cause === c).length,
  }));
    ...
    <section id="errorlog" className="section">
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        >
          ...
          {["Concept Gap", "Silly Mistake", "Timing"].map((s) => (
            <option key={s}>{s}</option>
          ))}
        </select>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        >
          {errors.length === 0
            ? 'No errors logged yet. Click "Log error" to start.'
            : "No matches."}
        </div>
      ) : (
            ...
            >
              <div
                style={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  background: CAUSE_COLORS[e.cause],
                  marginTop: 6,
                }}
              />
              <div style={{ flex: 1 }}>
                ...
                >
                  <div
                    style={{ fontWeight: 500, fontSize: "0.875rem", color: "var(--text-primary)" }}
                  >
                    {e.question}
                  </div>
      ...
      <Modal
        ...
        title={editing ? "Edit entry" : "Log an error"}
        width={480}
      >
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              placeholder="Describe the question or concept missed"
            />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            <div>
              ...
              <select
                ...
                value={form.type}
                onChange={(e) =>
                  setForm((f) => ({ ...f, type: e.target.value as ErrorEntry["type"] }))
                }
              >
                <option>Missed</option>
                <option>Guessed Right</option>
              </select>
            </div>
            <div>
              <label>Section</label>
              <select
                ...
                value={form.section}
                onChange={(e) =>
                  setForm((f) => ({ ...f, section: e.target.value as ErrorEntry["section"] }))
                }
              >
                <option>VARC</option>
                <option>DILR</option>
          ...
          <div>
            ...
            <select
              ...
              value={form.cause}
              onChange={(e) =>
                setForm((f) => ({ ...f, cause: e.target.value as ErrorEntry["cause"] }))
              }
            >
          ...
          <div>
            <label>One-line fix</label>
            <input
              className="input"
              value={form.fix}
              onChange={(e) => setForm((f) => ({ ...f, fix: e.target.value }))}
              placeholder="The specific action to prevent this again"
            />
          </div>
