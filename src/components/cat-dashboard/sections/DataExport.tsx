      ...
      try {
        ...
        if (data.kv_store?.length) {
          await supabase.from("kv_store").upsert(
            data.kv_store.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
        ...
        if (data.error_log?.length) {
          await supabase
            .from("error_log")
            .upsert(
              data.error_log.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
            );
          restored++;
        }
        if (data.mock_results?.length) {
          await supabase
            .from("mock_results")
            .upsert(
              data.mock_results.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
            );
          restored++;
        }
        if (data.topic_progress?.length) {
          await supabase.from("topic_progress").upsert(
            data.topic_progress.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
            { onConflict: "user_id,section,topic_name" },
          );
          restored++;
        }
        if (data.daily_activity?.length) {
          await supabase.from("daily_activity").upsert(
            data.daily_activity.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
        ...
        if (data.board_cards?.length) {
          await supabase
            .from("board_cards")
            .upsert(
              data.board_cards.map((r: Record<string, unknown>) => ({ ...r, user_id: user.id })),
            );
          restored++;
        }
    ...
    <section id="export" className="section">
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", marginTop: 4 }}>
          Your data lives in Supabase and is always backed up. Use JSON export for local copies or
          to migrate.
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          <div>
            ...
            <div style={{ color: "var(--text-muted)", fontSize: "0.8rem", lineHeight: 1.5 }}>
              Opens the browser print dialog. Use "Save as PDF" to get a snapshot of the current
              view.
            </div>
          </div>
