...
export default function Analytics() {
  ...
  const sectionData = (['VARC', 'DILR', 'QA'] as const).map((s) => ({
    name: s, value: errors.filter((e) => e.section === s).length,
  }));
  ...
  const radarData = (['QA', 'DILR', 'VARC'] as const).map((s) => {
    ...
    const done     = sec.filter((t) => ['Concept Done', 'Practice Done', 'Mastered'].includes(t.status)).length;
  ...
  const tooltipStyle = { contentStyle: { background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: 8, fontSize: '0.8rem' } };
    ...
    <section id="analytics" className="section">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        {(['overview', 'errors', 'mocks'] as const).map((t) => (
          <button key={t} className={`btn ${tab === t ? 'btn-amber' : 'btn-ghost'} btn-sm`} onClick={() => setTab(t)} style={{ textTransform: 'capitalize' }}>{t}</button>
        ))}
      </div>
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 300px), 1fr))', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 12, color: 'var(--text-secondary)' }}>30-day Activity</div>
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={actData} margin={{ left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="date" tick={{ fill: 'var(--text-muted)', fontSize: 9 }} interval={4} />
          ...
          {/* Topic radar */}
          <div className="card" style={{ padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 12, color: 'var(--text-secondary)' }}>Topic Mastery</div>
            <ResponsiveContainer width="100%" height={140}>
              <RadarChart data={radarData}>
                <PolarGrid stroke="var(--border)" />
                <PolarAngleAxis dataKey="subject" tick={{ fill: 'var(--text-muted)', fontSize: 12 }} />
                <Radar dataKey="Mastered" stroke="var(--amber)" fill="var(--amber)" fillOpacity={0.3} />
                <Radar dataKey="Progress" stroke="var(--teal)" fill="var(--teal)" fillOpacity={0.15} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      {tab === 'errors' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 300px), 1fr))', gap: 16 }}>
          {/* Cause breakdown */}
          <div className="card" style={{ padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 12, color: 'var(--text-secondary)' }}>By Root Cause</div>
          ...
          {/* Section breakdown */}
          <div className="card" style={{ padding: 20 }}>
            <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 12, color: 'var(--text-secondary)' }}>By Section</div>
            <ResponsiveContainer width="100%" height={160}>
      ...
      {tab === 'mocks' && (
        <div className="card" style={{ padding: 20 }}>
          <div style={{ fontWeight: 600, fontSize: '0.85rem', marginBottom: 12, color: 'var(--text-secondary)' }}>Mock Trend — Net Score (Overall)</div>
          {mockTrend.length < 2 ? (
            <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem', padding: '20px 0' }}>Log at least 2 overall mocks to see trend.</div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={mockTrend} margin={{ left: -20, right: 8 }}>
                ...
                <YAxis tick={{ fill: 'var(--text-muted)', fontSize: 10 }} />
                <Tooltip {...tooltipStyle} />
                <Line dataKey="net" stroke="var(--amber)" strokeWidth={2.5} dot={{ r: 4, fill: 'var(--amber)' }} name="Net Score" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
