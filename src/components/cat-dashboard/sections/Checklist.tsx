...
export default function Checklist() {
  const { value: items, setValue: setItems } = useKV<ChecklistItem[]>("checklist_v2", [
    {
      id: "c1",
      text: "Finish the embeddings ramp-up reading today - FlyRank starts Jul 1.",
      category: "Pre-Launch",
      done: false,
    },
    {
      id: "c2",
      text: "Set up error-log notebook before first live class on the 29th.",
      category: "Pre-Launch",
      done: false,
    },
    {
      id: "c3",
      text: "If laptop purchase is pending, close it this week.",
      category: "Pre-Launch",
      done: false,
    },
    {
      id: "c4",
      text: "Lock fixed wake time now before grind starts.",
      category: "Health",
      done: false,
    },
    {
      id: "c5",
      text: "If trying vocab habit, queue app/articles today.",
      category: "Study",
      done: false,
    },
  ]);
  ...
  const { value: customCategories, setValue: setCustomCats } = useKV<string[]>(
  ...
  const saveEdit = (id: string) => {
    ...
    setItems(
      list.map((i) => (i.id === id ? { ...i, text: editText.trim(), category: editCat } : i)),
    );
    setEditingId(null);
    addToast("Updated ✓");
  };
  ...
  return (
    <section id="tasks" className="section">
        ...
        {showCatForm && (
          ...
          >
            <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
              ...
              />
              <button className="btn btn-amber btn-sm" onClick={addCategory}>
                Add
              </button>
            </div>
            ...
            {addingCat === cat && (
              ...
              >
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  />
                  <button className="btn btn-amber btn-sm" onClick={() => addItem(cat)}>
                    Add
                  </button>
                </div>
              </motion.div>
                ...
                {editingId === item.id ? (
                  ...
                  >
                    ...
                    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                      ...
                      <button className="btn btn-amber btn-sm" onClick={() => saveEdit(item.id)}>
                        Save
                      </button>
      ...
      <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 8 }}>
        {allCats
          .filter((c) => !grouped.find((g) => g.cat === c))
          .map((cat) => (
            <button
              key={cat}
              className="btn btn-ghost btn-sm"
