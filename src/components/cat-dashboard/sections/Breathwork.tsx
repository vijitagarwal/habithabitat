...
export default function Breathwork() {
  const { value: breathLog, setValue: setBreathLog } = useKV<BreathLog>('breath_log', { streak: 0, total: 0, lastDate: '' });
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (isDone) {
        ...
        const newLog: BreathLog = { streak: newStreak, total: (breathLog.total || 0) + 1, lastDate: today };
        setBreathLog(newLog);
        markActivity(2);
  ...
  }, [running, rounds, phases, soundOn, breathLog, scale, stopBreath, setBreathLog, markActivity, addToast]);

  const phaseName = running ? curPhase?.l : 'Ready';
  const phaseColors: Record<string, string> = { Inhale: 'var(--teal)', Hold: 'var(--amber)', Exhale: 'var(--lav)', Ready: 'var(--text-muted)' };
  ...
  return (
    <section id="breathe" className="section">
      ...
      <div className="card" style={{ padding: 28, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
          {Object.entries(BREATH_PATTERNS).map(([key, p]) => (
            <button
              ...
              onClick={() => { if (!running) setPattern(key as PatternKey); }}
              disabled={running}
            >
              {p.name}
            </button>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {BREATH_ROUNDS.map((r) => (
            <button
              ...
              onClick={() => { if (!running) setRounds(r); }}
              disabled={running}
            >
              {r}
            </button>
        ...
        <div style={{ position: 'relative', width: 180, height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          >
            <span style={{ fontFamily: 'Space Grotesk', fontSize: '1.4rem', fontWeight: 700, color: circleColor }}>
              {running ? timeLeft : ''}
            </span>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          {!running ? (
            <button id="breath-start" className="btn btn-amber" onClick={startBreath} style={{ minWidth: 140 }}>
