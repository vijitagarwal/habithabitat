  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (msg.includes("Email not confirmed")) {
        msg =
          "Your email is not confirmed yet. Check your inbox for the confirmation link, or use Magic Link to sign in immediately.";
      } else if (msg.includes("Invalid login credentials")) {
        msg =
          "Invalid email or password. If you haven't set a password yet, use Magic Link or click Create Account.";
      }
      setErr(msg);
    } finally {
      setBusy(false);
  ...
  async function google() {
    ...
    try {
      ...
      if (msg.toLowerCase().includes("provider") || msg.toLowerCase().includes("unsupported")) {
        msg =
          "Google sign-in is not enabled in Supabase provider settings. Please use Magic Link or Email.";
      }
      setErr(msg);
      setBusy(false);
    }
  ...
  return (
    <div className="dark min-h-screen bg-background text-foreground">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        ...
        <div
          className="aurora-orb absolute top-1/3 -right-40 h-[26rem] w-[26rem] rounded-full bg-[oklch(0.72_0.18_55)]/15 blur-3xl"
          style={{ animationDelay: "-4s" }}
        />
        <div
          className="aurora-orb absolute -bottom-40 left-1/4 h-[24rem] w-[24rem] rounded-full bg-[oklch(0.62_0.2_155)]/15 blur-3xl"
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8 text-center">
            ...
            <div className="text-[10px] font-semibold tracking-[0.25em] text-muted-foreground">
              UNIFIED MISSION CONTROL
            </div>
            <h1 className="mt-1 text-2xl font-bold tracking-tight">Habit + CAT Dashboard</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Your holistic daily operating system
            </p>
          </div>
          ...
          <div className="card-glass rounded-2xl p-8 shadow-2xl">
            ...
            {mode === "magic" && (
              <p className="mb-4 rounded-xl bg-primary/10 p-3 text-xs text-muted-foreground">
                ...
                Get a sign-in link emailed to you — no password needed. Works with existing accounts
                too.
              </p>
            )}
            ...
            >
              <svg viewBox="0 0 24 24" className="h-4 w-4">
                ...
                <path
                  fill="#FBBC05"
                  d="M5.84 14.12A6.98 6.98 0 0 1 5.47 12c0-.74.13-1.45.36-2.12V7.04H2.18A11 11 0 0 0 1 12c0 1.78.43 3.46 1.18 4.96l3.66-2.84z"
                />
                <path
                  fill="#EA4335"
